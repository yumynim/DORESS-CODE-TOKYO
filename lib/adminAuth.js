/* =========================================================
   ...
   ■ 権限を2種類に分けている（2026-08-03〜）
     scope 'admin'   … ADMIN_CONSOLE_PASSWORD で発行。/console と /checkin の両方。
     scope 'checkin' … CHECKIN_PASSWORD で発行。/checkin だけ。
   当日のボランティアには CHECKIN_PASSWORD だけを渡す運用にすれば、その人が
   顧客の氏名・メールアドレス・お問い合わせ内容を見たり、会員全員にメールを
   送ったりはできなくなる（以前は合言葉が1つしか無く、入場確認を手伝って
   もらうだけで顧客情報の全権を渡すことになっていた）。
   CHECKIN_PASSWORD が未設定なら、今まで通り ADMIN_CONSOLE_PASSWORD だけで
   両方使える（設定し忘れても運用は止まらない）。
   ...
   ■ 署名鍵について（重要）
   以前は ADMIN_CONSOLE_PASSWORD をそのままHMACの鍵にしていたが、
   それだとトークンを1つ入手した人（例: 当日だけ手伝ってもらったボランティア）が、
   手元で合言葉を総当たりできてしまう。トークンは「有効期限（平文）.HMAC」という
   形なので、候補の合言葉でHMACを計算して一致するか試すだけでよく、
   HMAC-SHA256は非常に高速なため、ありがちな合言葉なら短時間で割れる。
   そこで:
...
function signingKey() {
  ...
  if (cachedKeySource !== password) {
    cachedKey = crypto.scryptSync(password, 'dct-admin-token-v1', 32);
