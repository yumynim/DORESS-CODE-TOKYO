...
// メール本文に直接埋め込む画像（受付コードのQRなど）。表示時に外部通信が発生しないため、
// 外部サービスの障害・同時アクセス制限に左右されず確実に表示される。
// サイズ上限を設けているのは、Console配信で万一巨大なdata URIが紛れ込んでも
// メール全体が肥大化しない（Gmail等は大きすぎるメールを「クリップ」して途中から
// 表示しなくなることがある）ようにするため。
const MAX_DATA_URI_LEN = 100000; // 約75KB相当のバイナリ（base64は元データの約4/3）
function isEmbeddableImageSrc(url) {
  if (isHttpUrl(url)) return true;
  return typeof url === 'string' && /^data:image\/(png|jpe?g|gif|webp);base64,/i.test(url) && url.length <= MAX_DATA_URI_LEN;
}
...
function renderBlocks(blocks) {
  ...
  for (const raw of list) {
    ...
    if (raw.type === 'heading') {
      ...
      if (!isEmbeddableImageSrc(raw.url)) continue;
