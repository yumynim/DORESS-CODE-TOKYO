...
/* Console（管理画面）の配信エディタが送ってくる「ブロック」配列を、
   ...
   ブロックの種類:
     - paragraph: { type: 'paragraph', text }  … 段落
...
function renderBlocks(blocks) {
  ...
  for (const raw of list) {
    ...
    if (raw.type === 'paragraph') {
      ...
      if (!text) continue;
      htmlParts.push(`<p style="margin:0 0 16px;">${escapeHtml(text).replace(/\n/g, '<br>')}</p>`);
      textParts.push(text);
 ...
 */
async function sendEmail({ to, subject, text, ctaLabel, ctaUrl, blocks, footerNote }) {
...
module.exports = { sendEmail, buildEmailHtml, textToHtmlParagraphs, renderBlocks, SITE_URL };
