...
### 2.（必須・今やっている最中・次回はここから再開）本番での通しテスト
...
**進捗（2026-08-05時点）**: `EVENT_DATE`設定・再デプロイ → 数量2で本物のカード決済（¥2,000） → 受付コード2つ発行 → 確認メールに2人分のQRが表示（1人目のQRだけ表示され2人目が欠けるバグを発見、その場で修正・再検証済み。原因は外部QR画像サービスの読み込み欠落で、以後はメールにQR画像を直接埋め込む方式に変更した）→ `/checkin`でハイフン抜き手入力の入場も確認済み → Squareで全額返金を**申請**（Refund実行）→ **返金が完了(COMPLETED)したかの確認が未実施**。

**次にやること**:
- [ ] Squareの取引詳細を開き、返金ステータスが「完了」になっているか確認する（申請直後は「処理中」のことがあり、COMPLETEDにならないとこちらのWebhookは反応しない）
- [ ] 完了していたら、以下5点を確認する:
  - マイページを開き直す → コードが消えて「返金済み」表示になっているか
  - 購入者（自分）宛てに「ご返金が完了しました」メール＋サイト内通知が届いているか
  - 運営宛てに【返金】メールが届いているか（すでに入場テスト済みなので「このお客様は入場済みです」の一文が入っているはず）
  - `/checkin`でそのQR・コードを読む/検索する → 赤い「返金済み・入場できません」＋お名前・メールアドレス・チケット名が出て入場させられないか
  - 通知ドロワーの「ご購入ありがとうございます」を開き直す → QRが消え、返金済みである旨の一文が付いているか
- [ ] （余裕があれば）機内モードで読み取り → 赤ではなく青い「通信できませんでした」が出るか

### 3.（必須・後日対応でよい・ユーザー了承済み）GitHubリポジトリをPrivateにする

**何のため**: 現在リポジトリが **Public（世界中から閲覧可能）**。`.env`本体や秘密鍵は含まれていないが、決済ロジック・DB構造・管理画面の作りまで誰でも読める状態は不要にさらしている。ユーザーからは「後日やる」との返答済み（2026-08-05）。
...
- ~~`ADMIN_TOKEN_SECRET`~~ → **設定済み（2026-08-05）**。ログイン状態を保つ「通行証」の署名鍵で、万一トークンが漏れても合言葉を推測される手がかりにならなくなった。設定後に一度全員ログアウトされる仕様だが対応済み。
- `CHECKIN_PASSWORD` … 当日ボランティアに渡す用の合言葉。未設定の今は`ADMIN_CONSOLE_PASSWORD`（管理者パスワードそのもの）でも`/checkin`に入れてしまう。設定すると、その人は入場確認しかできなくなり、会員の氏名・メール・問い合わせ内容には触れなくなる。**ユーザーは「必要になったら」と判断済み（2026-08-05）**。ボランティアに手伝いを頼む具体的な予定ができたら設定する。
