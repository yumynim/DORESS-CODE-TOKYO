...
- **特定商取引法に基づく表記（[`tokutei-shotorihiki.html`](../tokutei-shotorihiki.html)）の残りを埋める**（2026-08-01、支払い方法・引渡し時期・返品ポリシー等は記入済み。個人事業主の場合は住所・電話番号を「ご請求があれば遅滞なく開示します」に置き換えて省略できる、という消費者庁の扱いに沿って先に埋めた）:
  - ~~メールアドレス~~ 2026-08-01記入済み（`info@dress-code-tokyo.com`）
  - ~~販売事業者名~~ 2026-08-01一部記入（「DRESS CODE TOKYO（屋号）」と記載。**法人化しているか個人事業かが未確定**なため、確定次第正式な書き方に修正が必要。ユーザーへ確認済み: 事業者名はブランド名だけでは不可、個人事業の場合は本名も別途必須という法律上の制約を説明済み）
  - **運営統括責任者**（本名。省略不可、まだ未記入）
  - 上記が揃い、法人/個人の区別も確定したら`<meta name="robots">`の`noindex`を外す。正式公開前に一度、行政書士等の専門家に内容を確認してもらうことを推奨（このAIは法律専門家ではないため）。
