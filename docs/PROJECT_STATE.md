...
### 1.（最優先・必須）`supabase/schema_v15_checkin_idempotency.sql` を Supabase の SQL Editor で実行する

**何のため**: 実行するまで **当日の入場確認が動かない**。`api/admin-checkin.js` はこのSQLで作る関数（`checkin_entry` / `undo_checkin`）を呼ぶ作りになっているため、関数が無いと全部エラーになり、有効なチケットでも赤い「無効なコード」が出る。

このファイル1本で以下が全部そろう（v13が途中までしか適用されていなかった件の修復も含む。何度実行しても同じ結果になる）:
- まとめ買い（2枚以上）した人が、買った人数分ちゃんと入場できるようにする
- 会場のWi-Fiが遅くてスタッフが2回送信しても、人数が二重に消費されないようにする
- ブラウザから直接これらの関数を叩けないようにしつつ、サーバー（service_role）からは叩けるようにする
- 返金ステータス（`refunded`）を使えるようにする
- 過去のまとめ買いの人数を、カート内容から正しい値に直す

実行後、ファイル末尾のSELECTの結果が `service_role = true` / `anon`・`authenticated` = `false` になっていれば成功。
...
- 決済直後の「ありがとうございました」画面に受付コードとQRが出るか
- 確認メールが届き、そこにもコードとQRが載っているか
- マイページと通知ドロワーにも同じコードが出るか
- コードの形が `DCT-0927-N1-XXXX` になっているか（`0927`の部分が `EVENT` だったら`CURRENT_EVENT_ID`が未設定）
- `/checkin` でそのQRを読む → **「1 / 2 人目」** と出るか → もう一度読む → **「2 / 2 人目」** → 3回目で「入場済みです」
- ハイフンを抜いた手入力（`dct 0927 n1 xxxx`）でも通るか
- 「お名前・メールアドレスで探す」で自分を検索できるか
- 「取り消す」を2回押すと一覧から消えるか
- 最後にSquareで返金 → 受付コードが無効になり、運営に返金通知メールが届くか
