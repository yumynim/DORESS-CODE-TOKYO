...
1. **`supabase/schema_v10_event_sequence.sql`が未実行**: これを実行しないと受付コードの発行自体が失敗する（`next_entry_seq()`関数が存在しないため）。
2. **`/checkin`のチェックイン結果表示の仕様確認待ち**: 今はメールアドレスを表示しているが、ユーザーから「登録した名前の方がいいのでは」という話が出て、A（今のままメールでよい）／B（登録名に変える、または両方表示）のどちらか回答待ち。
3. **実地テスト一式が未実施**: 上記1が揃った後に、以下を通しで確認する必要がある
...
1. `supabase/schema_v10_event_sequence.sql`をSQL Editorで実行
2. `/checkin`のチェックイン結果表示（メールアドレス vs 登録名）についてユーザーに回答してもらい、必要なら実装
3. 実際の少額決済で一連の流れを通しテスト（上記2番）
4. `/checkin`の動作を通しテスト（上記2番）
5. （公開判断待ち）`tokutei-shotorihiki.html`の`noindex`を外すか
