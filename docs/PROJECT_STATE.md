...
### 1. ~~Square の Webhook に返金イベントを追加~~ → 完了（2026-08-04）

`refund.created` / `refund.updated` をSquare側で有効化済み。返金してもコードが無効化される状態になった。

**通しテストの進捗（2026-08-05時点）**: 数量2で購入 → 受付コード2つ発行 → メールにQR2つとも正常表示（1人目のQRが欠けるバグを発見・修正済み）→ `/checkin`でハイフン抜き手入力の入場も確認済み → **Squareで全額返金を申請済み、まだ完了(COMPLETED)通知が来ていない**。以下「返金後に確認すること」の5項目が**未確認のまま残っている**。次回セッションはここから再開する:
- Squareの取引詳細で返金ステータスが「完了」になっているか確認（申請直後は「処理中」のことがあり、COMPLETEDにならないとWebhookは動かない）
- 完了していたら下記5項目を確認

### 2.（必須・今やっている最中）本番での通しテスト

**何のため**: 決済からチケット発行、当日の受付までは複数のサービス（Square・Supabase・Resend・Vercel）をまたぐので、コードを読むだけでは繋がっているか確認できない。**イベント前に一度も通していない状態で当日を迎えるのが一番危ない。**

**先に**: 入場は開催日（`CURRENT_EVENT_ID`=0927 → 今年の9/27）しか受け付けないゲートが入ったため、テスト当日は Vercel の環境変数 **`EVENT_DATE` にテスト日の日付（例 `2026-08-05`）を設定して再デプロイ**する。テストが終わったら **必ず `EVENT_DATE` を削除**する（残すと当日9/27に入場できない）。

- **入場チケットを「数量2」で実際に購入する**（あとでSquareから返金すれば実質無料）
- 決済直後の「ありがとうございました」画面に**受付コードが2つ**（1人目・2人目）QR付きで出るか
- 確認メールに2つのコードとQRが「1人目／2人目」のラベル付きで載っているか
- マイページと通知ドロワーにも同じ2つのコードが出るか
- コードの形が `DCT-0927-N1-XXXX` と `DCT-0927-N2-XXXX` か（`0927`が `EVENT` なら`CURRENT_EVENT_ID`未設定）
- `/checkin` で1つ目のQRを読む → 緑「入場OK」＋「この購入の入場: 1 / 2 人」→ **同じQRをもう一度読む → オレンジ「入場済みです」**（これが1コード1回の証明）
- 2つ目のQRを読む → 「入場OK」＋「2 / 2 人（全員入場済み）」
- ハイフン抜きの手入力（`dct 0927 n1 xxxx`）でも通るか
- 「コードが分からない方を探す」で自分の名前を検索 → コードが2行出て、入場済み/未入場が正しいか
- チェックイン一覧から1件「取り消す」→ 一覧から消え、もう一度読むと入場OKに戻るか
- 機内モードにして読む → 赤ではなく**青い「通信できませんでした」**が出るか（余裕があれば）
- 最後にSquareで**全額**返金 → マイページからコードが消え、本人に返金メール＋通知が届き、運営に返金メールが届き、`/checkin`で読むと**赤い「返金済み・入場できません」＋お名前・メールアドレス・チケット名**が出るか（2026-08-04に「無効なコード」から改善）
- 返金後、購入時の「ご購入ありがとうございます」通知を開くと、QRが消えていて本文末尾に「※この購入は返金済みです。上記の受付コードは無効となり…」が付いているか
- 返金後、`/checkin`の「コードが分からない方を探す」でその人を検索すると、行は出るが「返金済み」と表示され入場ボタンが押せないか

### 3.（必須・今すぐ）GitHubリポジトリをPrivateにする

**何のため**: 現在リポジトリが **Public（世界中から閲覧可能）**。`.env`本体や秘密鍵は含まれていないが、決済ロジック・DB構造・管理画面の作りまで誰でも読める状態は不要にさらしている。ユーザー指示により対応中。

手順: [github.com/yumynim/DORESS-CODE-TOKYO/settings](https://github.com/yumynim/DORESS-CODE-TOKYO/settings) → 最下部 Danger Zone → Change repository visibility → Make private → リポジトリ名を入力して確定。Vercel連携は非公開化後もそのまま動く。

### 4.（任意・必要になったら）Vercel環境変数を2つ追加して再デプロイ

**何のため**（いずれも「今のままでも動く」— 当日ボランティアに手伝いを頼む具体的な予定ができたときに対応すればよい）:
- `CHECKIN_PASSWORD` … 当日ボランティアに渡す用の合言葉。未設定の今は`ADMIN_CONSOLE_PASSWORD`（管理者パスワードそのもの）でも`/checkin`に入れてしまう。**設定すると、その人は入場確認しかできなくなり、会員の氏名・メール・問い合わせ内容には触れなくなる。**
- `ADMIN_TOKEN_SECRET` … ログイン状態を保つ「通行証」の署名鍵。未設定の今は合言葉から鍵を生成している。設定すると、万一この通行証が漏れても合言葉そのものを推測される手がかりにならなくなる（総当たり攻撃への保険）。

Square Developer Dashboard → Webhooks → 登録済みのSubscriptionを編集 → `refund.created` と `refund.updated` にチェック。

### 5.（当日の準備・コードではなく運用）

**何のため**: システムが落ちたときに受付を止めないため。また、iPhoneではカメラ読み取りが使えないため。

- **Android端末を1台以上、受付に用意する**（iPhone・iPadはカメラでQRを読めない。手入力は可能）
- 当日の朝にスタッフ全員の端末で `/checkin` にログインしておく（ログイン状態は24時間で切れる）
- 端末の自動ロックを切り、明るさを最大にする
- `/console` の購入者一覧から**名簿を印刷しておく**（氏名・メール・受付コード・人数）。通信が止まっても受付を続けられる
- スタッフへの説明: 「青い『通信できませんでした』はお客様のせいではないので、追い返さずもう一度読む」「まとめ買いの人は人数分スキャンする（画面の『あと◯人分』を見る）」

### 6.（急ぎでない）TSUBASAさんの紹介文を入れる

`js/data.js` の `members` の TSUBASA に `desc` / `role` / `dept` を入れる。プレースホルダーがそのまま公開されていたため一旦空にしてあり、今は写真＋名前だけのカードになっている。

### 7.（急ぎでない・任意）

- `tokutei-shotorihiki.html` の条項を、可能なら一度専門家に見てもらう（今のものは類似サービスを調べて書いたドラフト）
- Google OAuth（Googleでログイン）の再開。今は `js/auth.js` の `GOOGLE_LOGIN_ENABLED = false` で非表示
- Sandboxテスト時代の「手続き中」テストデータを `purchases` から削除

## 完了した項目（2026-08-04・6巡目：管理者画面の整理＋通知・当日ゲートの穴）

- **`/console`の名前を「お知らせ投稿」→「管理者画面」に変更**（`admin-announcements.html`）: お知らせ配信だけでなく、お問い合わせ対応・購入者一覧・会員名簿まで載るようになり、名前と中身が合わなくなっていたため。ページタイトル・見出し・説明文を変更し、説明文から`/checkin`へのリンクも張った。ファイル名とURL（`/console`）は変えていない（変えるとブックマークが切れるため）。
- **個人宛て送信の宛先を、会員一覧から選ぶ方式に変更**（同上＋`api/admin-members.js`）: メールアドレスの手入力だと打ち間違いで「そのメールアドレスの会員が見つかりませんでした」になりがちだった。会員名簿から作った選択肢（お名前＋メールアドレス）を選ぶ形にし、一覧に無い相手へ送るとき用に「メールアドレスを直接入力」も残してある。
  - **この実装で自分が入れたバグを検出・修正済み**: 会員一覧の取得に一度でも失敗すると選択肢が「直接入力」1つだけになり、その値が選択状態として残るため、**あとで取得に成功しても手入力欄に貼り付いたまま一覧に戻らなかった**。取得成功時に「直接入力」を引き継がないよう修正（ブラウザで失敗→成功の順に再現させて確認済み）。
- **【重要】全員向けお知らせが、見ていないのに既読になっていた**（`js/notifications.js`）: 通知ドロワーを開いた瞬間に「ドレスコードからのお知らせ」を既読として記録していた。ドロワーは既定で「あなたへのお知らせ」タブが開くため、**購入通知だけ見て閉じた人は、全員向けの重要な連絡を一度も見ないまま赤いバッジが消え、二度と気づけない**状態だった。実際にそのタブを開いたときにだけ既読にするよう修正。
- **`/checkin`に「受付可能日」の表示を追加**（`api/admin-checkin.js`＋`checkin.html`）: 通しテストのために設定した`EVENT_DATE`を消し忘れたまま当日を迎えると、**最初のお客様をスキャンした瞬間に初めて入場できないと気づく**ことになる。ログイン直後に「本日◯月◯日は受付可能日です」または赤字で「⚠ 今日は入場を受け付けられません（受付可能日が◯月◯日に設定されています）」を出すようにした。`EVENT_DATE=any`のときも「日付チェックが無効です」と警告する。**当日の朝、スタッフがログインした時点で緑（通常表示）になっていることを必ず確認すること。**
- **「お支払い済みなのに受付コードが無い購入」の警告を会員名簿に追加**（`api/admin-members.js`＋`admin-announcements.html`）: Webhookが届かなかった・コード発行に失敗した場合、お客様は「払ったのに入場できない」まま当日を迎えるが、こちらは本人から連絡が来るまで気づけなかった。該当があれば会員名簿の先頭に赤い警告と対象者一覧を出す（返金済みは対象外）。

## 完了した項目（2026-08-04・5巡目レビュー：決済・メール・カート層）

これまでの4巡は受付（`/checkin`）まわりに集中しており、**決済・メール送信・カート・お問い合わせの層はほとんど読まれていなかった**。今回そこを端から読んで見つけた分。**コミット済み・push済み。**

- **【重大】メール送信の成否を握りつぶしていた**（`api/admin-announcements.js`）: `sendEmail()`は例外を投げず`true`/`false`を返す仕様なのに、呼び出し側が戻り値を見ずに`try{ await sendEmail(...); sent += 1; }`と書いていた。つまり**Resendが1通も受け付けていなくても画面には「メール送信 40件」と出る**。全員宛て（`sendBroadcastEmail`）に至っては結果を数えてすらおらず、完全に無言だった。運営は「送ったつもり」で当日を迎えることになる。戻り値を必ず数え、送れなかった件数を画面に警告として出すよう修正（全員宛て・チケット購入者宛て・決済未完了者宛て・個人宛ての4経路すべて）。
  - **関連して知っておくべきこと**: Resendの無料枠は**1日100通・月3000通**。会員が100人を超えると、1回の全員配信で1日の枠を使い切る。枠を超えた分は送信が失敗するが、サイト内のお知らせには届く。上の修正で「◯件送れませんでした」と出るようになったので、出たら時間をおいて送り直すか有料プランを検討すること。
- **受付コードの並び順が画面と食い違っていた**（`members-only.html`・`js/auth.js`）: DB側の発行処理は「文字数→文字列」の順で並べており、`N10`が`N2`より前に来ないよう明示的に対策されていた。ところがマイページ・サンクス画面・ヘッダーのドロワーの3箇所はすべて`.order('code')`（単純な文字列順）のままで、**10枚以上まとめ買いすると「2人目の受付コード: DCT-0927-N10-…」のように、画面のラベルとコードの連番がずれる**。同じ規則で並べ替えるよう3箇所とも修正（`N1 N10 N2 N20 N3` → `N1 N2 N3 N10 N20` になることを確認済み）。
- **お問い合わせの「ご用件」が自由入力のままサーバーに通っていた**（`api/contact.js`）: 自動返信メールの宛先は送信者が自由に指定できるため、問い合わせ本文は意図的に載せない作りになっていた（フィッシングの踏み台対策）。ところが`reason`は40文字まで自由入力を受け付け、それが自動返信の**件名と本文に入っていた**。同じ穴が半分開いたままだったことになる。`js/data.js`の選択肢（掲載依頼／出店依頼／その他お問い合わせ）に無い値は「その他お問い合わせ」として扱うよう修正。
- **カートの数量に上限が無かった**（`js/cart.js`）: サーバー側は1商品20個までなのに、カートの「＋」は無制限に押せた。21個以上にすると、レジに進んだ最後の最後で「数量が正しくありません」とだけ言われて先に進めなくなる（何が悪いのか分からない）。カート側でも20で止め、上限に達したら「＋」を押せなくした。
- **カートの商品名を未エスケープでinnerHTMLに入れていた**（同上）: カートの中身は`localStorage`にあり利用者自身が書き換えられるため、実害は自分自身に対してのみ（self-XSS）だが、サイトの他の箇所はすべてエスケープしている中でここだけ抜けていた。エスケープを追加。
- **Squareのエラー詳細をそのままブラウザに返していた**（`api/checkout.js`）: `detail: squareJson.errors`で、location設定やカタログの状態などが購入者に見える状態だった。ログにだけ残し、ブラウザには一般的な文言を返すよう変更。

### 検討したうえで、あえて直さなかったもの

- **`/api/checkout`にIPレート制限を付けること**: 他のAPIと揃えたくなるが、**当日会場のWi-Fiは多数の来場者が同じIPになる**ため、制限を入れると正規の購入者がまとめて弾かれる事故を作りかねない。ログイン必須で、Squareのidempotency keyもあるため見送った。

### 会員名簿・受付表示について、知っておくべき仕様

- **受付に出るお名前は本人の自己申告で、いつでも変更できる**（Supabase Authの`user_metadata.display_name`）。本人確認の補助として使うのはよいが、身分証の代わりにはならない。

## 完了した項目（2026-08-04・返金まわりの当日シミュレーション＋会員名簿）

ユーザーからの「購入キャンセルしたらQRが見えなくなるか」「キャンセルされた人は受付でそれと分かるか」という確認依頼を起点に、返金後の経路を追って修正した。あわせて`/console`に会員名簿を追加。**コミット済み・push済み。**

- **受付で「返金済み」だと分かるようにした**（`api/admin-checkin.js`＋`checkin.html`）: これまでは返金済みコードを読むと赤い「無効なコード」＋「そのコードは無効です（返金済み、または支払いが完了していません）」としか出ず、受付は目の前のお客様に何が起きたのか説明できなかった。コードが`entry_passes`に存在するのに入場処理が通らなかった場合、その理由（返金済み／支払いキャンセル／支払い未完了）と購入者のお名前・メールアドレス・チケット名まで調べて返すようにした。画面のバッジも「返金済み・入場できません」「お支払いキャンセル・入場できません」「お支払い未完了・入場できません」と出し分ける。
- **「コードが分からない方を探す」に返金済みの人も出るようにした**（同上）: 以前は`status='valid'`で絞っていたため、返金された方をお名前で検索しても「該当なし」としか出ず、受付は「入力ミスなのか、買っていないのか、返金されたのか」を判断できなかった。行としては出したうえで「返金済み」と明示し、入場ボタンを押せなくした。
- **返金後の通知本文にコードが残る問題を修正**（`api/square-webhook.js`）: 返金時にQR（`body_html`）は消していたが、本文（`body`）の「当日の受付コードは「DCT-…」です」という文字列はそのまま残っていた。通知を読んだ本人が有効なコードだと思い込んだまま来場してしまうため、本文末尾に「※この購入は返金済みです。上記の受付コードは無効となり、当日はご入場いただけません。」を追記するようにした（Squareの再送で二重に付かないようガード済み）。
- **`/console`に「会員名簿」セクションを追加**（`api/admin-members.js`（新規）＋`admin-announcements.html`＋`css/style.css`）: 会員を1人ずつ開くと、登録情報（お名前・メールアドレス・登録日・メール確認済みか・最終ログイン）／購入履歴と受付コード（入場済みか・返金で無効化されたか）／その人あてに配信したお知らせ全件（決済連動の自動送信か、運営が手で送ったかの区別付き）／その人からのお問い合わせと返信状況、がまとめて見られる。お名前・メールアドレス・受付コード・チケット名で検索できる。**追加した理由**: これまで「購入者一覧」「個人宛てに送った履歴」「お問い合わせ」が別セクションに分かれていて、「この人に今まで何を送ったか」を知るには3箇所を目で突き合わせる必要があり、二重に案内を送る／必要な連絡が漏れる原因になっていた。また、返金済みの購入はどのセクションにも出てこなかったが、名簿には出る。
  - **権限**: このAPIは氏名・メールアドレス・購入履歴・問い合わせ本文がまとめて出るため、`verifyAdminToken(token, 'admin')`で運営用の合言葉（`ADMIN_CONSOLE_PASSWORD`）を必須にしている。当日スタッフ用の`checkin`権限では開けない。**この条件を緩めないこと。**
  - 全員向けのお知らせは会員全員に届くため人ごとには持たず、件数だけ表示している。
- **検証済み**: ローカルでAPI応答をモックし、名簿の描画・検索（お名前／メールアドレス／受付コード／チケット名）・返金コードの「無効（返金）」表示、`/checkin`の3状態（返金済み／キャンセル／未完了）のバッジ出し分け、検索結果での返金済み行のボタン無効化、まで確認済み。JSの構文チェックも全ファイル通過。

### 直せない・仕様として残る点（当日の運用で知っておくこと）

- **すでに送信済みのメールに載っているQRは、あとから消せない**。QR画像は外部サービス（`api.qrserver.com`）のURLを`<img>`で読んでいるだけなので、返金してもお客様の受信箱では表示され続ける。メールというものの性質上どうにもならない。**対策として、コード自体はDB側で無効化済み・返金通知メールとサイト内通知が別途届く・受付で読むと「返金済み」と誰の分か表示される、の3段構えにしてある。** 当日「メールにQRが出ているのに入れない」と言われる可能性があることだけ、スタッフに共有しておくとよい。

## 完了した項目（2026-08-03・1人1コード方式＋日付ゲート＋4巡目レビュー）

- **schema_v15はSupabaseで実行済み**（確認SELECTの結果も正常）。その直後に下記の設計変更を行ったため、現役なのはv16。
- **受付コードを1人1コードに変更**（`supabase/schema_v16_entry_passes.sql`・未実行）: `entry_passes`テーブル（1行=1人分）、発行/入場/取り消しの3関数（全て行ロック付き）。発行はDB関数内で行うためWebhook再送・並行実行でも二重発行しない。乱数は`gen_random_uuid()`のバイト列由来。混載カート（出店1＋入場2）は1枚ずつ正しいカテゴリ（S/N）で発行。
- **入場の日付ゲート**（`api/admin-checkin.js`）: 開催日以外は入場を受け付けない。開催日は`EVENT_DATE`（`YYYY-MM-DD`または`any`）→無ければ`CURRENT_EVENT_ID`(MMDD)から当年の日付を導出。取り消し・履歴・検索は日付に関係なく使える。ゲートに引っかかったときは赤い「無効なコード」ではなく専用の「受付できません」表示。
- **4巡目レビュー（裏をかく視点＋当日シミュレーション）で見つけて修正**:
  - 3人組のQRを連続で読むと、処理中に読んだ2枚目が無言で捨てられ、画面に1人目の「入場OK」が残ったままになる（→処理が終わり次第自動で再挑戦する方式に）
  - 通信失敗後、お客様がQRをかざしたままだと再読み取りが永遠に始まらない（→失敗時にカメラの記憶をリセット。同じrequestIdを使うので二重入場にはならない）
  - 返金処理の順序: コード無効化に失敗しても再試行されない穴（→先に無効化、成功してから返金済みに更新。失敗時はSquareの再送でやり直される）
  - ログイン期限切れで黙ってログイン画面に戻り「直前の読み取りが記録されたか」不明だった（→理由と「記録されていません」を明示）
  - 取り消しの失敗が無言だった（→エラーを表示）
  - 一部返金メールに新方式のコード一覧が載っていなかった、まとめ買いの検索結果の選び方の案内不足、サンクス画面の「発行中」の自動再確認、マイページの「（入場済み）」表示、`EVENT_DATE`のスラッシュ書式許容、コードが N10>N2 と並ぶ問題
- レビューで**問題なしと確認**: entry_passesのRLS（他人のコードは読めない・書き込み不可）、3関数はブラウザから呼べない、plpgsqlの変数衝突なし、v16の再実行安全性、checkin.html⇔APIのフィールド整合、買い手のRLS読み取り、日付ゲートの適用範囲。
- 残存リスク（認識済み）: QR画像生成に外部サービス（api.qrserver.com）を使っておりコード文字列が外部に渡る（個人情報は含まない）。深夜0時（JST）をまたぐと入場ゲートが締まる（閉場が深夜に及ぶなら`EVENT_DATE`で対応）。

## 完了した項目（2026-08-03・3巡目の総点検で修正した10件）

3方向（受付スタック／決済〜Webhook／認証・サイト全体）のレビューで見つけたバグをすべて修正済み・push済み：

1. **二重カウント防止が実質機能していなかった**（`checkin.html`）: 通信失敗後の再送信で毎回新しいリクエストIDを作っていたため、サーバーは「同じ読み取り」と認識できなかった。失敗した読み取りのIDを覚えて再送時に使い回すように修正。
2. **受付2台で同じコードを交互に読むと余計に数える穴**（`schema_v15`）: 「直前の1件」だけ覚える方式を、数えた読み取りIDを配列（`checkin_request_ids text[]`）で全部覚える方式に変更。**v15はまだ未実行なのでDB側の互換問題なし。**
3. **QRを映しっぱなしにすると3秒ごとに勝手に人数を消費**（`checkin.html`）: 同じコードの再カウントには「一度カメラから外して（1.2秒）もう一度かざす」という明確な動作を要求するように変更。
4. **遅れて届いた決済失敗通知が支払い済みを上書き**（`api/square-webhook.js`）: カード1回目失敗→2回目成功の順で決済したとき、遅延・再送されたFAILED通知が`paid`を`canceled`に戻し、コードが見えなくなる＋キャンセルメールが届く事故。状態遷移を限定（canceled←initiatedのみ、paid←initiated/canceledのみ）。これで**返金済み→paidの復活（新コード再発行）も同時に塞がった**。
5. **一部返金で注文全体のコードを失効させていた**（同上）: 返金額が全額未満なら自動失効させず、運営に【要対応】メールで判断を委ねる方式に変更。
6. **返金が購入者本人に一切通知されなかった**（同上）: 本人宛てメール＋サイト内通知を追加。購入時通知に残っていた無効なQRも消すようにした。
7. **確認メールの送信失敗を誰も検知できなかった**（`lib/mailer.js`＋webhook）: `sendEmail`が成否を返すようにし、購入確認メールが送れなかったら運営に【要対応】メールを出す。
8. **サンクス画面が「一番新しい購入」を監視していた**（`api/checkout.js`＋`members-only.html`）: 別タブで次の注文を始めるとそちらを監視して40秒タイムアウトする問題。戻りURLに購入行ID（`&pid=`）を含め、その注文だけを監視するように変更。
9. **確認メール再送ボタンのリセット漏れ**（`js/auth.js`）: ボタンが2つ（ログイン/新規登録フォーム）あるのに1つしかリセットしておらず、アドレスを打ち直しても古いアドレス宛てに再送されうる問題。全ボタンをリセット。
10. **パスワード再設定リンクで新パスワード画面が出ないことがあるレース**（`js/auth.js`）: `PASSWORD_RECOVERY`イベントを購読前に取りこぼすと画面が出ない。URLの`#type=recovery`を読み込み時点で退避して確実に開くようにした。

そのほか: `/checkin`のトークンをURLクエリからヘッダー送信に変更（Vercelのアクセスログに残さない）、手入力のあいまい照合を今回イベントに限定＋上限追加（1000行超で照合漏れする前に対処）、検索から「入場させる」を押したときの人数表示の遅れ・無反応を修正、`render.js`の死にコード（gallery描画）削除、`data.js`の事実と逆のコメント修正、`schema_v15`にv10未適用時の安全弁を追加。
...
- `supabase/schema_v11_notifications_update_guard.sql`実行済み（通知の更新を既読フラグだけに制限）。
- **攻撃者視点での棚卸しを実施し、見つかった穴を修正**（コミット`acf9ecf`）。最も重大だったのは「1,000円の入場チケットを買いながら商品名だけ『出店料』と送ると、5,000円の出店者用受付コード(S)が発行される」というもの。決済金額はSquareがカタログ価格で請求するため金銭的な不正は無かったが、出店枠を安く取得できてしまう状態だった。商品名・価格・カテゴリはすべて`lib/catalog.js`（サーバー側の正本）から引くように変更。他に、チェックインの競合（同じコードで2人入場できた）、`/console`と`/checkin`の権限分離、Webhook署名鍵が空文字のときに検証を素通りする問題、問い合わせ自動返信が第三者へのフィッシング中継に使える問題、`admin-login`・`check-email`のレート制限、`/api/env-check`の公開などを修正。詳細はコミットメッセージ参照。
...
- `supabase/schema_v12_purchases_insert_lockdown.sql`実行済み（会員が`purchases`に任意の行を作れる穴を塞いだ）。
- まとめ買い対応と返金対応を追加（コミット`4dd3590`）: `purchases.quantity`/`checked_in_count`を追加し、買った枚数分だけ入場できるようにした。カウントはDB関数`checkin_entry()`が行ロック付きで増やすので、受付が複数台でも二重に数えない。返金は`refund.created`/`refund.updated`を受けて`status='refunded'`＋`entry_code`をnullにし、運営にメールで知らせる（入場後の返金だった場合はその旨も本文に出す）。

## 既知の運用上の注意（未対応・仕様として許容中）

- **iPhoneではカメラでのQR読み取りができない**（Safari・iOS版Chromeとも`BarcodeDetector`非対応。iOSではChromeを入れても中身はSafariなので変わらない）。手入力は使えるので運用は可能。手入力はハイフンや空白を無視して照合する。画面にもその旨を明記済み。
- **受付コードの生成を外部サービス（api.qrserver.com）に任せている**。QR画像のURLに受付コードが含まれるため、その事業者にはコードが渡る。無料・ライブラリ不要を優先した判断。読み取り自体は端末内で完結するので、当日この事業者が落ちていても手入力で運用できる。
- **スタッフの操作ログが無い**。誰がチェックイン・取り消しをしたかは記録されない（トークンに個人の識別情報を持たせていないため）。合言葉を共有する方式である以上、個人までは追えない。
- **受付コードは「持っていれば入れる」もの**。QRのスクリーンショットを他人に送れば、その人も入れてしまう（買った人数分まで）。当日は`/checkin`に出る氏名と本人を照らし合わせる運用でカバーする。
- **`/checkin`の権限でも、受付に来た人の氏名とメールアドレス、チェックイン済み一覧は見える**。本人確認に必要なため。完全に隠したい情報がある場合はこの方式では足りない。
