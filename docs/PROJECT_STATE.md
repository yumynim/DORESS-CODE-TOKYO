...
0-A. **`supabase/schema_v13_checkin_count.sql`が未実行**（最優先）: 実行するまで、(1)まとめ買い（2枚以上）した人の2人目以降が入場できない、(2)返金しても受付コードが有効なまま、という状態が続く。`api/admin-checkin.js`は既にこのSQLで作る関数（`checkin_entry`/`undo_checkin`）を呼ぶ形になっているため、**実行するまで入場確認そのものが動かない**。SQL Editorで実行すること。
   - あわせてSquare Developer Dashboard → Webhooks で `refund.created` と `refund.updated` を購読対象に追加すること（追加しないと返金してもコードが無効化されない）。

0-B. **（任意・推奨）Vercel環境変数の追加**:
   - `CHECKIN_PASSWORD` — 当日ボランティア用の合言葉。設定すると、その人は`/checkin`だけ使えて、顧客の氏名・メール・問い合わせ内容は見られなくなる。**未設定だと今まで通り1つの合言葉で全権**なので、当日スタッフに手伝ってもらうなら設定を強く推奨。
   - `ADMIN_TOKEN_SECRET` — ログイントークンの署名鍵（`openssl rand -hex 32`等）。未設定でも動くが、設定するとトークンが漏れても合言葉自体は割り出されなくなる。
   - どちらも設定後にRedeployが必要。
...
- **iPhoneではカメラでのQR読み取りができない**（Safari・iOS版Chromeとも`BarcodeDetector`非対応）。手入力は使えるが15文字あるので、当日はAndroid端末を1台以上用意するか、手入力前提で人員を配置すること。手入力はハイフンや空白を無視して照合するようにしてある。
...
- **スタッフの操作ログが無い**。誰がチェックイン・取り消しをしたかは記録されない（トークンに個人の識別情報を持たせていないため）。
