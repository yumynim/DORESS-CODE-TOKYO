...
### 3.（推奨）Vercel環境変数を2つ追加して再デプロイ

**何のため**:
- `CHECKIN_PASSWORD` … 当日ボランティアに渡す用の合言葉。**これを設定しないと、入場確認を手伝ってもらうだけで、その人が `/console` から全顧客の氏名・メール・問い合わせ内容を見られ、会員全員へのメール送信までできてしまう。** 設定すれば、その人は入場確認しかできなくなる。
- `ADMIN_TOKEN_SECRET` … ログイン状態を保つトークンの署名鍵（`openssl rand -hex 32` の出力でよい）。設定しておくと、万一トークンが漏れても合言葉そのものを推測されなくなる。

### 4.（推奨）Square の Webhook に返金イベントを追加

**何のため**: 返金しても受付コードが有効なままだと、返金した相手がそのまま入場できてしまう。コード側は対応済みなので、Squareに通知を送ってもらう設定だけが残っている。
