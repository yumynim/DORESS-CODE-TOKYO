...
0-A. **`supabase/schema_v12_purchases_insert_lockdown.sql`が未実行**（セキュリティ・最優先）: 実行するまで、ログイン中の会員がブラウザから`purchases`テーブルに直接、任意の`price`/`ticket_name`/`entry_code`/`square_order_id`を持つ行を作れる。特に**まだ発行されていない受付コードを先に埋めて、正規購入者へのコード発行を失敗させる嫌がらせ**ができる状態。SQL Editorで実行すれば塞がる。
...
1. **`supabase/schema_v12_purchases_insert_lockdown.sql`をSQL Editorで実行**（上記0-A。最優先）
