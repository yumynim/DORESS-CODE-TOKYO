...
**全額返金でなければ、受付コードは自動的に無効化されない。** まとめ買い（出店料＋入場×2など）を一部だけ返金したとき、機械的にコードを消すと返金していない人まで入場できなくなるため、あえて自動処理せず運営の判断に委ねる設計になっている。

- **全額返金**（購入1件をまるごと）: Squareで通常どおり全額払い戻しするだけでよい。その購入のコードは全部自動で無効化される。
- **1枚だけ返金**（まとめ買いの一部だけ）:
  1. Squareでその1枚分の金額だけ**一部返金**する。
  2. 運営宛てに【要対応】一部返金メールが届く（購入IDと現在のコード一覧が記載）。
  3. Supabase の SQL Editor で、無効化したいコードを指定して実行する:
     ```sql
     update entry_passes set status = 'revoked' where code = '該当コード';
     ```
     **すでに入場済み（`checked_in_at`に日時が入っている）コードは絶対に無効化しない**（会場に入っている人を締め出すことになる）。
  4. `/checkin`の「コードが分からない方を探す」でそのコードを検索し、「返金済み」表示になっていれば完了。他のコードは影響を受けない。
